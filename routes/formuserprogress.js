...
router.get('/getbyid/:id', async(req, res, next) => {
  ...
  const id = req.params.id;
    ...
    if(formuserprogress == null) {
        res.status(StatusCodes.NOT_FOUND).json({message: "FormUserProgress not found"})
        return};
    res.status(StatusCodes.OK).json({formuserprogress});
});
...
router.get('/getall', async(req, res, next) => {
  ...
  const formuserprogresses = await FormUserProgress.findAll({where : {isDeleted: false}});
    if(formuserprogresses.length == 0) {
        res.status(StatusCodes.NOT_FOUND).json({message: "No FormUserProgresses found"})
        return};
    res.status(StatusCodes.OK).json({formuserprogresses});
});
...
router.put('/update/:id', async(req, res, next) => {
  ...
  const id = req.params.id;
    ...
    if(formuserprogress.isDeleted == true) {
        res.status(StatusCodes.BAD_REQUEST).json({message: "FormUserProgress already deleted"})
        return};
    await FormUserProgress.update(req.body.FormUserProgress, {where: {id: id}});
    res.status(StatusCodes.OK).json({message: "FormUserProgress updated"});
});
...
router.delete('/delete/:id', async(req, res, next) => {
  ...
  const id = req.params.id;
    ...
    res.status(StatusCodes.OK).json({message: "FormUserProgress deleted"});
});
...
router.delete('/harddelete/:id', async(req, res, next) => {
  ...
  const id = req.params.id;
    ...
    res.status(StatusCodes.OK).json({message: "FormUserProgress deleted from database"});
});
...
router.post('/create', async(req, res, next) => {
  ...
  if(!FormUserProgress.incomingCorrectlyFilled(req.body.FormUserProgress)){
        res.status(StatusCodes.BAD_REQUEST).json({message: "FormUserProgress not correctly filled"});
        return;
    }
