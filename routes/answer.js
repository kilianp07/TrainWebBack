...
router.get('/getbyid/:id', async(req, res, next) => {
  ...
  if(answer == null) {
    res.status(StatusCodes.StatusCodes.NOT_FOUND).json({message: "Answer not found"})
    return};
  res.status(StatusCodes.StatusCodes.OK).json({answer});
});
...
router.get('/getall', async(req, res, next) => {
  ...
  if(answers.length == 0) {
    res.status(StatusCodes.StatusCodes.NOT_FOUND).json({message: "No answers found"})
    return};
  res.status(StatusCodes.StatusCodes.OK).json({answers});
});
...
router.post('/create', async(req,res,next) => {
  ...
  if (Answer.incomingCorrectlyFilled(req.body.Answer) == false) {
     ...
     res.status(StatusCodes.StatusCodes.CREATED).json({createdAnswer, message: "Answer created"})
});
...
router.delete('/delete/:id', async(req, res, next) => {
  ...
  const id = req.params.id;
    ...
    res.status(StatusCodes.StatusCodes.OK).json({message: "Answer already deleted"});
      return
    }
    await Answer.update({isDeleted : true}, {where: {id: id}});
    res.status(StatusCodes.StatusCodes.OK).json({message: "Answer deleted"});
});
...
router.delete('/harddelete/:id', async(req, res, next) => {
  ...
  const id = req.params.id;
    ...
    res.status(StatusCodes.StatusCodes.OK).json({message: "Answer deleted from database"});
});
...
router.put('/update/:id', async(req, res, next) => {
  ...
  res.status(StatusCodes.StatusCodes.OK).json({message: "Answer updated"});
});
